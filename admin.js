...
async function uploadFile(type) {
    ...
    if (!name || !audioFile || !photoFile) {

        alert(
            "Name, photo and audio file select pannunga"
        );
    ...
    try {
        ...
        const audioPath =
            `${type}/${Date.now()}_${audioFile.name}`;

        const photoPath =
            `${type}/photos/${Date.now()}_${photoFile.name}`;
        ...
        alert(
            "Music + Photo Uploaded Successfully!"
        );
